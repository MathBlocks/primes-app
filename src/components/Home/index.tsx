...
const MintRandomPrimeButton: FC = () => {
  ...
  const { send, state } = useContractFunction(Primes, 'mintRandomPrime', {
  ...
  return (
    <div>
      <button
        onClick={() => {
          send(merkleProof, { value: (1e17).toString() }).catch((error) => {
            console.error(error)
          })
        }}
      >
...
const MintRandomPrime: FC = () => {
  ...
  return <div>{contracts && account && <MintRandomPrimeButton />}</div>
}
