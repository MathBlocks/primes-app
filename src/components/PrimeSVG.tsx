...
const generateDefs = (
  ...
  const interval = Math.floor(100 / (primeAttributes.length - 1))
  return (
    <defs>
      <linearGradient id="g1" x1="0" y1="0" x2="100%" y2="0">
        {/*<stop offset="0%" stopColor={fg} />*/}
        {primeAttributes.map(({ fill }, idx) => (
          <stop offset={`${interval * idx}%`} stopColor={fill} key={idx}>
            {idx !== 0 && (
              <animate
                attributeName="stop-color"
                values={`${fill};none;${fill}`}
                dur="5s"
                begin={idx}
...
export const PrimeSVG: FC<{
  ...
  return (
    ...
    >
      <style>{STYLE}</style>
      {generateDefs(tokenId, fg, primeAttributes)}
        ...
        {shells.map((shell, shellIdx) => (
          <g key={shellIdx} className="shell">
            <animateTransform
              ...
              dur={`${shells.length === 1 ? 60 : 5 * (shellIdx + 1)}s`}
              repeatCount="indefinite"
            />
            <path
              ...
              style={{ stroke: prime ? fg : `url(#g1)`, strokeWidth: 2 }}
            />
      ...
      <g
        id="text"
        fill={fg}
        transform={`translate(${(259 - 13 * chars.length) / 2}, 121)`}
      >
        {chars.map((n, idx) => (
          <g key={n} transform={`translate(${13 * idx})`}>
            {numberSVG(n)}
          </g>
        ))}
      </g>
...
export const getSVGDataURI = (
  ...
  const svgString = btoa(
    renderToStaticMarkup(
      <PrimeSVG tokenId={tokenId} primeAttributes={primeAttributes} />,
    ),
  )
